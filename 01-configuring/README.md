...
# Stock Market Historical Price Dataset (US Stocks & ETFs)

Данный датасет содержит исторические рыночные данные по акциям и ETF фондов США.
Источник — открытый набор данных с платформы Kaggle:

**Price Volume Data for All US Stocks & ETFs**

Набор включает дневные котировки тысяч инструментов за многолетний период и предназначен для анализа временных рядов, построения торговых стратегий, машинного обучения и финансовой аналитики.

## Структура данных

После загрузки структура имеет вид:
...
Каждый файл соответствует одному тикеру.

В рамках проекта все файлы были объединены в единый CSV:

```
combined_all_stocks.csv
```
...
| high   | DECIMAL | Максимальная цена за день  |
| low    | DECIMAL | Минимальная цена за день   |
| close  | DECIMAL | Цена закрытия              |
| volume | BIGINT  | Объём торгов               |
| ticker | VARCHAR | Биржевой тикер инструмента |
